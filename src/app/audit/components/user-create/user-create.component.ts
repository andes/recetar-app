...
export class UserCreateComponent implements OnInit, OnDestroy {
    ...
    onSave(): void {
        if (this.userForm.valid) {
            ...
            // Andes: { profesion: { nombre, codigo }, matriculacion: [{ matriculaNumero, fin }] }
    ...
    isUsernameDisabled(): boolean {
        return !this.hasProfessionalRole() && !this.hasOnlyAuditorRole(); // Deshabilitar cuando no es profesional ni auditor
    }
    ...
    private autocompleteFields(data: any): void {
        if (data) {
            ...
            if (isProfessional) {
                ...
                // Autocompletar nombre: se usa razonSocial (titular legal) como nombre del usuario;
                // si no existe, se usa denominacion como fallback.
    ...
    private setupDocumentValidation(): void {
        this.usernameSearchSubject.pipe(
            ...
            takeUntil(this.destroy$),
            switchMap(documento => {
                // Si no es rol profesional, no buscar en Andes
