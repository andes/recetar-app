...
export class NewPrescriptionComponent implements OnInit, OnDestroy {
    ...
    userOrganizations: SubOrganization[] = [];
    organizationsLoading = false;
    userProfesionGrado: Array<{ profesion: string; codigoProfesion: string; numeroMatricula: string }> = [];

    patientObrasSociales: { nombre: string; codigoPuco: string }[] = [];
    ...
    ngOnInit(): void {
        ...
        if (userId) {
            this.userService.getUserById(userId).pipe(
                ...
                this.userProfesionGrado = user.profesionGrado || [];
            });
        }
    ...
    get showPendingBanner(): boolean {
        return this.pendingPrescription !== null;
    }
    ...
    resumePendingPrescription(): void {
        if (!this.pendingPrescription) { return; }
        const payload = this.pendingPrescription.payload as Record<string, any>;

        if (payload.patient) {
            const p = payload.patient;
            this.draftService.setPatient(new Patient(
                p.lastName || '',
                p.firstName || '',
                p.sex || '',
                p.status || p.estado || '',
        ...
        if (payload.supplies && Array.isArray(payload.supplies)) {
            const medications: MedicationItem[] = payload.supplies.map((s: any) => ({
                snomedConcept: s.snomedConcept || s.supply?.snomedConcept || undefined,
                supply: s.supply ? {
                    id: Number(s.supply.code?.value) || 0,
                    nombre: s.supply.name || '',
                    presentacion: s.supply.firstPresentation || '',
                    precio: s.supply.price ?? 0,
                    potencia: s.supply.power || '',
                    droga_descrip: s.supply.activePrinciple || '',
                    accion_descrip: s.supply.actionDesc || '',
                    codigoDeBarras: s.supply.barCode ? [s.supply.barCode] : [],
                    snomed: s.supply.snomedConcept?.conceptId || '',
                } as any : undefined,
                quantity: Number(s.quantity) || 1,
                packageQuantity: Number(s.quantityPresentation) || 0,
                ...
                obraSocial: s.obraSocial || undefined,
            }));
            this.draftService.setMedications(medications);
        }
    ...
    private loadUserOrganizations(): void {
        ...
        this.userService.getUserById(userId).pipe(
            takeUntil(this.destroy$),
            catchError(() => of({ organizaciones: [] } as any)),
            finalize(() => this.organizationsLoading = false)
        ).subscribe((user) => {
            this.userOrganizations = user.organizaciones || [];
            this.preselectOrganization(userId);
        });
    }

    private preselectOrganization(userId: string): void {
        const lastOrgId = localStorage.getItem(`recetar_last_org_${userId}`);
        if (lastOrgId) {
            const found = this.userOrganizations.find(o =>
                (o._id && o._id === lastOrgId) || o.nombre === lastOrgId
            );
            if (found) {
                this.draftService.setOrganizacion(found);
                return;
            }
        }
        if (this.userOrganizations.length === 1) {
            this.draftService.setOrganizacion(this.userOrganizations[0]);
        }
    }

    selectOrg(org: SubOrganization): void {
        this.draftService.setOrganizacion(org);
        const userId = this.authService.getLoggedUserId();
        const key = org._id || org.nombre;
        localStorage.setItem(`recetar_last_org_${userId}`, key);
    }

    orgDisplayFn = (org: SubOrganization): string => org.nombre;

    orgSubtitleFn = (org: SubOrganization): string => org.direccion || '';

    orgTrackByFn = (_index: number, org: SubOrganization): string => org._id || org.nombre;
    ...
    get canFinalize(): boolean {
        ...
        if (this.draft.type === 'prescription' && !this.draft.organizacion) { return false; }
    ...
    selectPatient(patient: Patient | FrequentPatient): void {
         if ('_id' in patient) {
            ...
            this.loadPatientObrasSociales(p.dni || '', p.sex || '');
        } else {
            ...
            this.patientsService.getPatientByDni(fp.dni).subscribe({
                next: (patients) => {
                    if (patients.length > 0) {
                        ...
                        this.draftService.setPatient(p);
                        this.loadPatientObrasSociales(fp.dni, fp.sex);
                    }
                }
            });
        }
    ...
    private loadPatientObrasSociales(dni: string, sexo: string): void {
        if (!dni || !sexo) {
            this.patientObrasSociales = [];
            return;
        }
        this.patientsService.getPatientOSByDni(dni, sexo).pipe(
            takeUntil(this.destroy$),
    ...
    getSidebarCertEndDate(): Date {
        const d = this.draft.certificateData;
        if (!d?.startDate || !d?.cantDias) { return d?.startDate || new Date(); }
        const end = new Date(d.startDate);
        end.setDate(end.getDate() + d.cantDias - 1);
        return end;
    }
    ...
    finalizePrescription(): void {
        ...
        const professional = {
            ...
            profesionGrado: this.userProfesionGrado,
            ...(this.draft.organizacion ? {
                organizacion: {
                    _id: this.draft.organizacion._id,
                    nombre: this.draft.organizacion.nombre,
                }
            } : {}),
        };
    ...
    hasDifferentObraSocial(item: MedicationItem): boolean {
        if (!item.obraSocial?.nombre) { return false; }
        const patientOS = this.draft.patient?.obraSocial?.nombre;
        return item.obraSocial.nombre !== patientOS;
    }

    getItemObraSocialLabel(item: MedicationItem): string {
        return item.obraSocial?.nombre || this.draft.patient?.obraSocial?.nombre || '';
    }
