...
export class AuditUsersComponent implements OnInit, AfterContentInit, OnDestroy {
    ...
    @ViewChild('tbSort', { static: true }) tbSort = new MatSort();
    ...
    private loadUsers(offset: number = 0, limit: number = 10, searchTerm?: string): void {
    ...
    getUserRolesTooltip(user: User): string {
        ...
        return user.roles.map(role => this.translateRole(role.role)).join(', ');
    }
    ...
    getAvailableRoleOptions(): Role[] {
        ...
        return this.availableRoleOptions.filter(role => {
            if (hasPharmacistRole && this.rolesService.isProfessionalRole(role.role)) { return false; }
            if (hasProfessionalRole && this.rolesService.isPharmacistRole(role.role)) { return false; }
            return true;
        });
    }
    ...
    saveAllChanges(): void {
        ...
        this.usersService.updateUser(this.editingUser._id, updateData).subscribe({
            ...
            error: (err) => {
                this.loadingUsers = false;
                this.notificationService.error(err || 'Error al guardar cambios');
            }
        });
    }
