...
export class DispenseItemComponent {
    ...
    private get andes(): AndesPrescriptions | null {
        return this.isAndesItem ? this.prescription as AndesPrescriptions : null;
    }
    ...
    get professionalName(): string {
        const andes = this.andes;
        if (andes) {
            return (andes.profesional as unknown as { nombre?: string })?.nombre || '';
        }
    ...
    get isNearExpiry(): boolean {
        ...
        const andes = this.andes;
        const date = andes
            ? new Date(andes.fechaRegistro || andes.fechaPrestacion || now).getTime()
            : new Date(this.local?.date || now).getTime();
