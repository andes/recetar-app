...
export interface DispenseReplacement {
    name: string;
    quantity: number;
...
export class DispenseService implements OnDestroy {
    ...
    private queryTrigger$ = new Subject<DispenseFilters>();
    ...
    constructor(
        ...
        this.queryTrigger$.pipe(
            switchMap((filters) => this.buildRequest(filters).pipe(
    ...
    load(filters: DispenseFilters): void {
        ...
        this.queryTrigger$.next(this.filters);
    }

    private buildRequest(filters: DispenseFilters): Observable<DispenseListResponse> {
        const { searchTerm, status, sexo, dateFrom, dateTo, offset = 0, limit = DEFAULT_LIMIT } = filters;

        if (searchTerm && /^\d{6,8}$/.test(searchTerm)) {
            return this.loadByDni(searchTerm, { status, sexo, dateFrom, dateTo, offset, limit });
        }

        return this.loadGeneric({ status, sexo, dateFrom, dateTo, offset, limit, searchTerm });
    }

    private emptyResponse(): DispenseListResponse {
        return {
    ...
    dispense(prescription: MixedPrescription, replacement?: DispenseReplacement): Observable<DispenseResult> {
        ...
        return this.http.patch<Prescriptions>(
            `${environment.API_END_POINT}/prescriptions/${prescription._id}/dispense`,
            body,
        ).pipe(
            map((updated) => {
                this.upsertPrescription(this.prescriptionsAdapter.adapt(updated));
                return { success: true };
            }),
            catchError((error: unknown) => of({
                success: false,
    ...
    private loadByDni(dni: string, filters: {
        ...
        if (filters.sexo) {
            params = params.set('sexo', filters.sexo);
        }
        if (filters.dateFrom) {
