...
export class DocumentsHomeComponent implements OnInit, OnDestroy {
    ...
    openDrawer(item: MixedPrescription): void {
        ...
        if (this.isAndesPrescription(item)) {
            const a = item;
            this.drawerDate = (a.fechaPrestacion || a.fechaRegistro || '') as string;
            ...
            this.drawerDate = (p.date || '') as string;
            ...
            if (supply) {
                const name = (supply.supply as any)?.name || '';
                ...
                this.drawerMedDetail = [power && unity ? `${power}${unity}` : '', firstPres ? `· ${firstPres}` : '', qty > 1 ? `· ${qty} unidades` : ''].filter(Boolean).join(' ');
