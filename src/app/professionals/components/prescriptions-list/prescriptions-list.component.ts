...
export class PrescriptionsListComponent implements OnInit, AfterContentInit, OnDestroy {
    ...
    canPrint(prescription: MixedPrescription): boolean {
        if (this.isAndesPrescription(prescription)) {
            ...
            return this.normalizeStatus(prescription.estadoActual.tipo) !== 'VENCIDA';
        } else {
            return (prescription.professional.userId === this.authService.getLoggedUserId()) && this.normalizeStatus(prescription.status) !== 'VENCIDA';
        }
    }
