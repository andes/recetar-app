...
import { FrequentMedication, MedicationItem, PrescriptionItemResult, toFrequentMedication, fromFrequentMedication } from '@shared/models/medication.types';
...
export class MedicationSearchComponent implements OnInit, OnDestroy, OnChanges {
    ...
    toggleOptions = [
        { value: 'commercial' as const, label: 'Comercial', icon: 'prescription' },
        { value: 'generic' as const, label: 'Generico', icon: 'science' },
    ];
    ...
    confirmMedication(): void {
        ...
        if (this.simplified) {
            ...
            const r = this.pendingMedication;
            if (r.kind === 'commercial') {
                ...
                this.recordFrequencyAndNotify(toFrequentMedication(r), item, r.entry.nombre);
            } else {
                const item: MedicationItem = {
                    snomedConcept: r.concept,
                    quantity,
                    packageQuantity: 1,
                    diagnostic: '',
                    indication: '',
                ...
                this.recordFrequencyAndNotify(toFrequentMedication(r), item, r.concept.term);
            }
            return;
        }
