...
export class PrescriptionsPublicService {
  ...
  getPrescriptions(params): Observable<boolean> {
    return this.http.get(`${environment.API_END_POINT}/prescriptions-public`, { params }).pipe(
  ...
  getById(id: string): Observable<Prescriptions> {
    return this.http.get<Prescriptions>(`${environment.API_END_POINT}/prescriptions-public/${id}`);
  }
  ...
  dispense(prescriptionId: string, pharmacistId: string): Observable<boolean> {
    ...
    return this.http.patch<Prescriptions>(`${environment.API_END_POINT}/prescriptions-public/${params.prescriptionId}/dispense`, params).pipe(
  ...
  cancelDispense(prescriptionId: string, pharmacistId: string): Observable<boolean> {
    var params = { 'prescriptionId': prescriptionId, 'pharmacistId': pharmacistId };
    return this.http.patch<Prescriptions>(`${environment.API_END_POINT}/prescriptions-public/${params.prescriptionId}/cancel-dispense`, params).pipe(
      tap((updatedPrescription: Prescriptions) => this.updatePrescription(updatedPrescription)),
      mapTo(true)
    );
  }
  ...
  getFromDniAndDate(params: { patient_dni: string, dateFilter: string }): Observable<boolean> {
    return this.http.get<Prescriptions[]>(`${environment.API_END_POINT}/prescriptions-public/find/${params.patient_dni}&${params.dateFilter}`).pipe(
      tap((prescriptions: Prescriptions[]) => this.setPrescriptions(prescriptions)),
      map((prescriptions: Prescriptions[]) => prescriptions.length > 0)
    );
  }
  ...
  getByUserId(userId: string): Observable<Boolean> {
    return this.http.get<Prescriptions[]>(`${environment.API_END_POINT}/prescriptions-public/get-by-user-id/${userId}`).pipe(
      tap((prescriptions: Prescriptions[]) => this.setPrescriptions(prescriptions)),
      mapTo(true)
    );
  }
  ...
  newPrescription(prescription: Prescriptions): Observable<Boolean> {
    return this.http.post<Prescriptions[]>(`${environment.API_END_POINT}/prescriptions-public`, prescription).pipe(
  ...
  editPrescription(prescription: Prescriptions): Observable<Boolean> {
    return this.http.patch<Prescriptions>(`${environment.API_END_POINT}/prescriptions-public/${prescription._id}`, prescription).pipe(
      tap((updatedPrescription: Prescriptions) => this.updatePrescription(updatedPrescription)),
      mapTo(true)
    );
  }
  ...
  deletePrescription(prescriptionId: string): Observable<Boolean> {
    return this.http.delete<Prescriptions>(`${environment.API_END_POINT}/prescriptions-public/${prescriptionId}`).pipe(
