...
| 2026-09-09 | Búsqueda de medicamentos **solo comercial**: se elimina el toggle comercial/genérico de `MedicationSearchComponent` (quita `searchMode`, `toggleOptions`, `onModeChange` y la rama SNOMED de `doSearch`; la búsqueda usa siempre `vademecumService.searchMedications`). Al confirmar un item comercial ya no se enriquece con SNOMED (`getByConceptId` + fallback sintetizado); el `MedicationItem` queda solo con `supply` + detalle. Se elimina el método `getByConceptId` de `SnomedSuppliesService` (el servicio y `get()` se conservan para `professional-form`). Se quitan el binding del toggle y el bloque `::ng-deep ui-toggle` del SASS. Texto de ayuda en nueva-receta actualizado a "por nombre comercial". Aplica a alta/edición de recetas y al reemplazo en dispensación (mismo componente compartido). | `shared/components/medication-search/`, `prescription/create/`, `pharmacists/`, `services/snomedSupplies` |
| 2026-09-09 | `MedicationSearchComponent` desacoplado del tipo unión `PrescriptionItemResult`: el estado interno (`filteredMedications`, `pendingMedication`) pasa a `VademecumEntry`; se eliminan todas las ramas `kind === 'commercial'/'generic'` de `enterEditMode`, `selectMedication`, `selectFrequentMedication`, `confirmMedication` y `trackByMed`. Al editar un item genérico heredado (solo `snomedConcept`, sin `supply`) ya no se arma el detalle: se vuelve a la búsqueda para elegir un comercial. Los medicamentos frecuentes legacy `kind='generic'` se filtran (`loadFrequentMedications`). `MedicationItemComponent` amplía su input para aceptar también `VademecumEntry` (rama `'nombre' in` en `medKind`/`medName`/`medDetail`/`medPrice`), cambio backward-compatible. | `shared/components/medication-search/`, `shared/ui/medication-item`, `prescription/create/`, `pharmacists/` |
| 2026-09-09 | `MedicationItemComponent.medDetail` muestra ahora la **droga + potencia** delante de la acción y presentación: `droga_descrip + potencia | accion_descrip | presentacion` (helper `drugLabel`, solo si hay `droga_descrip`). Aplica a resultados de búsqueda, detalle de selección y diálogo de dispensación. | `shared/ui/medication-item` |
| 2026-09-09 | Se elimina `snomedConcept` de los **modelos de prescription locales**. Frontend: `MedicationItem` deja de tener `snomedConcept` (medication.types.ts); `medication-adapter` genera payload solo comercial (sin `buildSnomedConcept` ni rama snomed-only); `new-prescription` (resume + `supplyToMedicationItem` + sidebar) y `document-tabs` (draft/historial) dejan de leer `snomedConcept`; `interfaces/prescriptions.ts` quita `replacedMedication.snomedConcept`; dispensa local (`dispense-home`, `dispense.service`, `dispense-confirm-dialog`) deja de manejar reemplazo por concepto. Backend `recetar-api` módulo `prescriptions`: se quita `snomedConcept` de `prescription.types.ts`, `prescription.dto.ts`, `prescription.model.ts`, del merge en `prescription.service.ts` y de `prescription.openapi.yaml`; test de controller actualizado. Se conservan intactos los flujos externos: recetas ANDES (`andesPrescriptions`, `dispense-home` rama Andes, `PrescriptionItemResult`/`FrequentMedication` con kind genérico) y el autocomplete magistral SNOMED (`professional-form`, `SnomedSuppliesService.get`, `interfaces/supplies`). | `prescription/create/`, `shared/models/medication.types`, `pharmacists/`, `interfaces/prescriptions`, `recetar-api/prescriptions` |
