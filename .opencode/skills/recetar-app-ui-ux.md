...
### 1.1 Paleta Primaria: "Azul Petróleo"

Seleccionada como el identificador central de Recetar para establecer una diferenciación competitiva frente a soluciones existentes (evitando azul genérico o tonos púrpuras) y transmitir credibilidad médica sin caer en estéticas frías u hospitalarias.

| Token Semántico | Valor HEX | Uso Recomendado / Contexto UI | WCAG |
|---|---|---|---|
| `color-primary-50` | `#E8F3FA` | Superficies hover suave, elementos tipo chip, fondo informativo claro. | AAA |
| `color-primary-100` | `#C5DFF0` | Fondos de componentes seleccionados, contenedores tipo badges. | AAA |
| `color-primary-200` | `#8BBEDD` | Delimitadores visuales, bordes y separadores de énfasis sutil. | AA |
| `color-primary-400` | `#1D6F9E` | Iconografía activa, hipervínculos, estado informativo base. | AA |
| `color-primary-500` | `#0B4F82` | Color institucional base. Botones primarios, navegación activa. | AAA |
| `color-primary-600` | `#083B63` | Estados hover en botones primarios, cabeceras de sección. | AAA |
| `color-primary-700` | `#052844` | Estados activos avanzados (active), texto de énfasis oscuro. | AAA |

Alias SASS:
| `$primary` | `$primary-dark` (hover/active) | `$primary-light` (variante clara) |
|---|---|---|
| `color-primary-500` | `color-primary-600` | `color-primary-400` |

### 1.2 Paleta Secundaria: Teal. Acento

**Criterio de Aplicación:** No tiene responsabilidades informáticas o de estado. Su uso queda exclusivamente para componentes de acción e hitos interactivos (FABs, toggles, stepper activo) para evitar competir con la jerarquía del azul petróleo.

| Token Semántico | Valor HEX | Uso Recomendado / Contexto UI | WCAG |
|---|---|---|---|
| `color-secondary-50` | `#E5F5F4` | Fondos de chips secundarios. | AAA |
| `color-secondary-100` | `#B3E5E2` | Resaltados contextuales, badges secundarios. | AA |
| `color-secondary-400` | `#0C8C87` | Botones secundarios de soporte, FABs. | AA |
| `color-secondary-500` | `#0C6E69` | Estados hover secundarios, links de soporte. | AAA |
| `color-secondary-600` | `#09524E` | Estados activos secundarios (active). | AAA |

Alias SASS:
| `$secondary` | `$secondary-dark` (hover/active) | `$secondary-light` (variante clara) |
|---|---|---|
| `color-secondary-500` | `color-secondary-600` | `color-secondary-400` |
...
La selección tipográfica responde a la necesidad de garantizar una lectura óptima en pantallas con cierta complejidad de datos técnicos (principio activo, matrículas profesionales y códigos comerciales).

### 4.1 Fuentes Oficiales

| Fuente | Uso | Google Fonts |
|---|---|---|
| **Inter** | Tipografía principal. Rendimiento en interfaces, altura de x elevada para facilitar lectura con mayúsculas y minúsculas. Se diferencia del estándar (Roboto) del resto de las interfaces. | https://fonts.google.com/specimen/Inter |
| **JetBrains Mono** | Tipografía de datos. Exclusiva para cadenas de caracteres (códigos de recetas, CUIL, matrículas profesionales, nomencladores). | https://fonts.google.com/specimen/JetBrains+Mono |

### 4.2 Escala y Jerarquía (Tokens de Texto)

| Token | Font | Size | Weight | Line-height | Uso |
|---|---|---|---|---|---|
| `font-display` | Inter | 28px (1.75rem) | 600 (Semibold) | 1.2 | Títulos de pantalla |
| `font-heading-l` | Inter | 22px (1.375rem) | 600 (Semibold) | 1.3 | Encabezados de sección mayor |
| `font-heading-m` | Inter | 18px (1.125rem) | 500 (Medium) | 1.4 | Cabeceras de tarjetas (cards) |
| `font-heading-s` | Inter | 15px (0.9375rem) | 500 (Medium) | 1.4 | Etiquetas de grupo interno |
| `font-body-l` | Inter | 16px (1.0rem) | 400 (Regular) | 1.6 | Texto narrativo principal |
| `font-body-m` | Inter | 14px (0.875rem) | 400 (Regular) | 1.5 | Textos de formularios y etiquetas de datos |
| `font-body-s` | Inter | 12px (0.75rem) | 400 (Regular) | 1.4 | Metadatos y marcas de tiempo |
| `font-label-l` | Inter | 14px (0.875rem) | 500 (Medium) | 1.0 | Texto interno de botones y chips |
| `font-code` | JetBrains Mono | 13px (0.8125rem) | 400 (Regular) | 1.5 | Códigos de barras y validaciones numéricas |
...
$primary: #0B4F82
$primary-dark: #083B63
$primary-light: #1D6F9E

// Colores secundarios
$secondary: #0C6E69
$secondary-dark: #09524E
$secondary-light: #0C8C87
...
// Hover
$hover-bg: rgba($primary, 0.10)

// Sombras
$shadow-sm: 0 1px 3px rgba(11, 79, 130, 0.08)
$shadow-lg: 0 4px 16px rgba(11, 79, 130, 0.16)
...
- Usar `MatCard` con `$radius-md` (8px) y `elevation-1`.
...
6. **Colores de boton:** usar `color="primary"` (azul institucional) o `color="accent"` (teal secundario). Para warning/amarillo usar `class="mat-warn"`. Para error/rojo destructivo usar `class="mat-error"`. **No usar** `color="warn"` de Material (rojo por defecto) ni clases inventadas (`btn-error`, `btn-warn`, `error-icon`). Los estilos de `.mat-warn` y `.mat-error` ya estan definidos globalmente en `styles.sass` para iconos, outlined, tonal y filled.
7. **Layout con FlexLayoutModule:** para alinear y distribuir botones u otros elementos en fila o columna, usar `fxLayout`, `fxLayoutAlign` y `fxLayoutGap` en lugar de custom CSS (`display: flex`, `justify-content`, `gap`, etc.).
